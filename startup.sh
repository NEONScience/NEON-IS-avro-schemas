...
GH_RUNNER_VERSION="2.291.1"
# get actions binary
curl -o actions-runner.tar.gz --location "https://github.com/actions/runner/releases/download/v${GH_RUNNER_VERSION}/actions-runner-linux-x64-${GH_RUNNER_VERSION}.tar.gz"
mkdir /actions-runner
mkdir /actions-runner-tmp
...
# configure runner
RUNNER_ALLOW_RUNASROOT=1 /actions-runner/config.sh --unattended --replace --work "/actions-runner-tmp" --url "$REPO_URL" --token "$ACTIONS_RUNNER_TOKEN" --labels gce-runner
